...
def prepare():
    body = urllib.parse.urlencode(generate_request_param(api_prepare))
    return lfasr_post(api_prepare, body, base_header)
...
def generate_request_param(apiname, taskid=None, slice_id=None):
    ...
    signa = base64.b64encode(hmac.new(secret_key.encode(), hl.hexdigest().encode(), sha1).digest())
    signa = signa.decode()
...
def post_multipart_formdata(strparams, content):
    ...
    L.append('')
    # use ISO-8859-1 decode
    content_decode = content.decode(encoding)
    L.append(content_decode)
    L.append('--' + BOUNDARY + '--')
    L.append('')
    body = CRLF.join(L)
